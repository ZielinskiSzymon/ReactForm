...
const SubmissionView = ({ selectedKurs, setSelectedKurs, existingCategories = [], handleCourseAction }) => {
	...
	const publishEnabled = (() => {
		if (!selectedKurs) return false
		if (selectedKurs.published) return true
		if (typeof selectedKurs.ilosc === 'number') {
			return qualifiedCount >= selectedKurs.ilosc
		}
		return false
	})()
	...
	const handleKwalifikacjaChange = async (kursId, daneId, currentStatus) => {
		...
		if (newStatus && typeof maxSeats === 'number' && qualifiedCount >= maxSeats) {
			return
		}
		...
		try {
			...
			setSubmissions((prevSubmissions) =>
				prevSubmissions.map((osoba) => (osoba.id === daneId ? { ...osoba, zakwalifikowano: newStatus } : osoba))
			)

			setQualifiedCount((prev) => (newStatus ? prev + 1 : Math.max(0, prev - 1)))
	...
	const handlePublishToggle = async () => {
		if (!selectedKurs || !selectedKurs.id) return
		try {
			setPublishing(true)
			const newState = !selectedKurs.published
			await publishCourse(selectedKurs.id, newState)
			if (handleCourseAction) await handleCourseAction()
			await loadSubmissions({ ...selectedKurs, published: newState })
		} catch (err) {
	...
	return (
		<>
			...
			<div className='mb-3 d-flex gap-2 justify-content-end'>
				...
				<button
					className={`btn btn-${selectedKurs?.published ? 'outline-warning' : 'primary'} btn-sm`}
					onClick={handlePublishToggle}
					disabled={publishing || !publishEnabled}
					title={selectedKurs?.published ? 'Cofnij publikację wyników' : 'Opublikuj wyniki'}>
					{publishing ? 'Trwa...' : selectedKurs?.published ? 'Cofnij publikację' : 'Publikuj wyniki'}
				</button>
				{!selectedKurs?.published && !publishEnabled && (
					<div className='small text-muted align-self-center'>
						Publikacja dostępna gdy wszystkie miejsca są zajęte. ({qualifiedCount} / {selectedKurs?.ilosc ?? '—'})
					</div>
				)}
			</div>
			...
			{selectedSubmissionId && (
				<SubmissionDetailsModal submissionId={selectedSubmissionId} onClose={() => setSelectedSubmissionId(null)} />
			)}
		</>
	)
}
