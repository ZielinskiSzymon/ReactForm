...
import { fetchQualifiedCount, fetchSubmissionCount } from '../../services/formularzService'
...
const CourseSidebar = ({
	...
	useEffect(() => {
		...
		return () => window.removeEventListener('participant-count-changed', handler)
	}, [])
		...
		<div className='col-md-4'>
			<div className='mb-4 pb-3 border-bottom'>
				<div className='d-flex flex-wrap gap-2 align-items-center'>
					<span className='text-muted small fw-bold me-2 text-uppercase'>Kategorie:</span>
					...
					{top4Kategorie.length < kategorie.length && <span className='text-muted small ms-2'>...i inne</span>}
				</div>
			</div>

			<div className='card border shadow-sm' style={{ borderColor: '#dee2e6' }}>
				<div className='card-header bg-white fw-bold text-secondary border-bottom'>Dostępne Kursy</div>
				<div className='list-group list-group-flush'>
					{filteredCourses.length > 0 ? (
						filteredCourses.map((kurs) => {
							...
							const percent = kurs.ilosc ? Math.min(100, Math.round((accepted / kurs.ilosc) * 100)) : 0
							...
							return (
								<button
									...
									className={`list-group-item list-group-item-action py-3 ${
										selectedKurs?.id === kurs.id ? 'active fw-bold' : 'text-dark'
									}`}
									onClick={() => {
									...
									<div className='d-flex justify-content-between align-items-center'>
										<div className='me-2 text-truncate' style={{ maxWidth: '65%' }}>
											<div className='d-flex align-items-center gap-2'>
												<span>{kurs.nazwa}</span>
												{kurs.published && (
						...
						<div className='p-3 text-muted small'>Brak kursów w kategorii "{selectedKategoria}".</div>
					)}
				</div>
			</div>
		</div>
